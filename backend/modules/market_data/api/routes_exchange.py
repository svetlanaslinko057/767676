...
@router.get("/token/{symbol}/markets")
async def get_token_markets(
    symbol: str,
    include_ticker: bool = Query(False, description="Include current price data")
):
    """
    Где торгуется токен - список всех бирж и инструментов.
    
    Returns all exchanges and trading pairs for a given token.
    """
    await instrument_registry.sync_all()
    ...
    for venue in Venue:
        instruments = instrument_registry.list_instruments(venue, None)
        
        for inst in instruments:
            if inst.base == symbol_upper:
                market_info = {
                    "venue": venue.value,
                    ...
                    "base": inst.base,
                    "quote": inst.quote,
                    "market_type": inst.contract_type.value if inst.contract_type else "spot",
                    "status": inst.status
                }
                
                # Optionally include ticker
                if include_ticker:
                    try:
...
async def get_token_price_across_venues(symbol: str):
    """
    Цена токена на всех биржах.
    
    Get token price from all available exchanges for comparison.
    """
    symbol_upper = symbol.upper()
    prices = []
    errors = []
    ...
    for venue in Venue:
        ...
        try:
            ticker = await provider.get_ticker(symbol_upper)
            prices.append({
                "venue": venue.value,
    ...
    valid_prices = [p["last"] for p in prices if p.get("last")]
    avg_price = sum(valid_prices) / len(valid_prices) if valid_prices else None
    
    # Find best bid/ask
    best_bid = max([p.get("bid") for p in prices if p.get("bid")] or [None])
    best_ask = min([p.get("ask") for p in prices if p.get("ask")] or [None])
    
    return {
        "ts": _now_ms(),
        "symbol": symbol_upper,
        "venues_checked": len(Venue),
        "venues_available": len(prices),
