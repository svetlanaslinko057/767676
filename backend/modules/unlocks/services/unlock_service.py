...
class UnlockService:
    ...
    async def get_upcoming_unlocks(
        ...
        # Support both 'date' and 'unlock_date' field names
        query = {
            '$or': [
                {'date': {'$gte': now.isoformat(), '$lte': end_date.isoformat()}},
                {'unlock_date': {'$gte': now, '$lte': end_date}}
            ]
        }
        
        if min_value_usd:
            query['$or'][0]['amount_usd'] = {'$gte': min_value_usd}
            query['$or'][1]['unlock_value_usd'] = {'$gte': min_value_usd}
        
        if min_percent:
            query['$or'][0]['percent_supply'] = {'$gte': min_percent}
            query['$or'][1]['unlock_percent'] = {'$gte': min_percent}
        
        cursor = self.unlocks.find({}, {'_id': 0})
        unlocks = await cursor.to_list(length=500)
        
        # Filter manually to handle both date formats
        result = []
        for u in unlocks:
            unlock_date = u.get('date') or u.get('unlock_date')
            if not unlock_date:
                continue
            ...
            if now <= unlock_date <= end_date:
                # Apply value filters
                value_usd = u.get('amount_usd') or u.get('unlock_value_usd', 0)
                percent = u.get('percent_supply') or u.get('unlock_percent', 0)
        ...
        result.sort(key=lambda x: x.get('date') or x.get('unlock_date', ''))
        return result[:limit]
