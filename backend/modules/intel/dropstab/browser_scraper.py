...
class DropstabBrowserScraper:
    ...
    async def scrape_all(self, headless: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("[Dropstab] Playwright not installed! Run: pip install playwright && playwright install chromium")
        ...
        logger.info(f"[Dropstab] Headless={headless}, Proxy={'configured' if self._proxy else 'none'}")
        ...
        try:
            async with async_playwright() as p:
                ...
                logger.info(f"[Dropstab] Launching Chromium browser...")
                browser = await p.chromium.launch(**launch_args)
                logger.info(f"[Dropstab] Browser launched successfully")
                ...
                for i, target in enumerate(TARGETS):
                    ...
                    logger.info(f"[Dropstab] [{i+1}/{len(TARGETS)}] Opening: {url}")
                    ...
                    try:
                        ...
                        logger.info(f"[Dropstab] [{label}] Page loaded, status={status}")
                        
                        if response and response.status >= 400:
                            logger.warning(f"[Dropstab] [{label}] HTTP error: {response.status}")
                            page_results[label] = {"status": "error", "http_status": response.status}
                            continue
                        ...
                        logger.debug(f"[Dropstab] [{label}] Waiting {wait_time:.1f}s for dynamic content...")
                        ...
                        for selector in ["text=Load more", "text=Show more", "button:has-text('Next')"]:
                            try:
                                ...
                                if await btn.is_visible(timeout=1000):
                                    await btn.click()
                                    logger.info(f"[Dropstab] [{label}] Clicked: {selector}")
                                    await asyncio.sleep(random.uniform(1.5, 2.5))
                            except:
                                pass
                        
                        # Count captured items for this page
                        page_captured = len([c for c in self.captured if c["label"] == label])
                        page_elapsed = time.time() - page_start
                        page_results[label] = {"status": "ok", "captured": page_captured, "elapsed_sec": round(page_elapsed, 1)}
                        logger.info(f"[Dropstab] [{label}] Captured {page_captured} items in {page_elapsed:.1f}s")
                        ...
                        page_elapsed = time.time() - page_start
                        logger.error(f"[Dropstab] [{label}] FAILED after {page_elapsed:.1f}s: {type(e).__name__}: {e}")
                        page_results[label] = {"status": "failed", "error": str(e), "elapsed_sec": round(page_elapsed, 1)}
                ...
                await browser.close()
                logger.info(f"[Dropstab] Browser closed")
                
        except Exception as e:
            logger.error(f"[Dropstab] CRITICAL browser error: {type(e).__name__}: {e}")
            return {
        ...
        logger.info(f"[Dropstab] COMPLETE: {len(self.captured)} items from {len(TARGETS)} pages in {elapsed:.1f}s")
        for label, pr in page_results.items():
            logger.info(f"[Dropstab]   {label}: {pr}")
