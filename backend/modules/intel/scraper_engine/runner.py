...
class ScraperRunner:
    ...
    async def discover_domain(self, domain: str, headless: bool = True) -> Dict[str, Any]:
        """
        Discover endpoints from any domain using browser.
        Universal discovery for user-added sources.
        
        Args:
            domain: Domain to discover (e.g., "dextools.io")
            headless: Run browser headless
            
        Returns:
            Discovery result with captured endpoints
        """
        # Clean domain
        ...
        # Build target URLs
        targets = {
            "main": f"https://www.{domain}",
            "api": f"https://{domain}/api",
        }
        
        # Add common pages for different types of sites
        common_pages = [
            "/tokens", "/coins", "/pairs", "/markets",  # DEX/Market sites
            "/funding", "/fundraising", "/rounds",       # Funding sites
            "/unlocks", "/vesting",                      # Unlock sites
            "/news", "/feed",                            # News sites
        ]
        
        for page_path in common_pages:
            targets[page_path.strip("/")] = f"https://www.{domain}{page_path}"
        
        logger.info(f"[Runner] Browser discovery for {domain}...")
        
        result = await browser_discovery.discover(domain, targets, headless)
        
        # Register discovered endpoints
        if result.get("endpoints"):
            for ep in result["endpoints"]:
                try:
                    endpoint_registry.upsert(ep)
                except Exception as e:
